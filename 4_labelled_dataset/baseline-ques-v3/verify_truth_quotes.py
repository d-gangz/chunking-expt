"""
Enhanced verification script to check if all source quotes from base_ground_truth.json exist in Jake's data.json.

Input data sources: 4_labelled_dataset/baseline-ques-v3/base_ground_truth.json, 1_transcripts/jake/data.json
...
def extract_source_quotes(ground_truth_path: Path) -> List[Tuple[str, str, str]]:
    """Extract all source quotes from base_ground_truth.json."""
    with open(ground_truth_path, "r", encoding="utf-8") as f:
        ground_truth_data = json.load(f)

    quotes = []
    for entry in ground_truth_data["entries"]:
        question_id = entry["question_id"]
        for quote in entry["source_quotes"]:
            quotes.append(
                (question_id, quote["transcript_title"], quote["quoted_text"])
            )
...
def map_transcript_titles(
    source_title: str, available_transcripts: Dict[str, str]
) -> str:
    ...
    for title in available_transcripts.keys():
        if (
            clean_source.lower() in title.lower()
            or title.lower() in clean_source.lower()
        ):
            return title
...
def verify_quotes(
    ground_truth_path: Path, data_path: Path, debug_mode: bool = False
) -> List[QuoteMatch]:
    """Verify all source quotes exist in transcript data."""
    print("🔍 Starting ground truth source quote verification...")

    # Load data
    source_quotes = extract_source_quotes(ground_truth_path)
    transcript_data = load_transcript_data(data_path)

    print(
        f"📊 Analyzing {len(source_quotes)} source quotes across {len(transcript_data)} transcripts"
    )
    if debug_mode:
    ...
    for i, (question_id, transcript_title, quoted_text) in enumerate(source_quotes, 1):
        if debug_mode:
            print(
                f"\n[{i:2d}/{len(source_quotes)}] 🔍 DETAILED ANALYSIS: {question_id}"
            )
            print(
                f"Quote preview: \"{quoted_text[:100]}{'...' if len(quoted_text) > 100 else ''}\""
            )
        else:
            print(f"[{i:2d}/{len(source_quotes)}] {question_id}...", end=" ")
        ...
        if debug_mode:
            ...
            for j, chunk in enumerate(clean_chunks, 1):
                print(
                    f"  Chunk {j}: \"{chunk[:80]}{'...' if len(chunk) > 80 else ''}\""
                )
            ...
            for phrase, strategy in search_phrases[:3]:  # Show first 3
                print(
                    f"  {strategy}: \"{phrase[:60]}{'...' if len(phrase) > 60 else ''}\""
                )
...
def generate_report(results: List[QuoteMatch]) -> None:
    ...
    print("\n" + "=" * 80)
    print("📋 GROUND TRUTH SOURCE QUOTE VERIFICATION REPORT")
    print("=" * 80)
    ...
    transcript_not_found = sum(
        1 for r in results if r.match_type == "transcript_not_found"
    )
    ...
    print(
        f"  ✅ Exact matches: {exact_matches} ({exact_matches/total_quotes*100:.1f}%)"
    )
    print(
        f"  ⚠️  Partial matches: {partial_matches} ({partial_matches/total_quotes*100:.1f}%)"
    )
    print(
        f"  ❌ Not found in transcript: {not_found} ({not_found/total_quotes*100:.1f}%)"
    )
    ...
    if partial_matches > 0:
        high_conf = sum(
            1 for r in results if r.match_type == "partial" and r.confidence == "high"
        )
        med_conf = sum(
            1 for r in results if r.match_type == "partial" and r.confidence == "medium"
        )
        low_conf = sum(
            1 for r in results if r.match_type == "partial" and r.confidence == "low"
        )
    ...
    print(f"\n📝 QUESTION ANALYSIS:")
    questions_with_all_quotes_found = 0
    for question_id, question_quotes in question_results.items():
        all_found = all(q.found for q in question_quotes)
        if all_found:
            questions_with_all_quotes_found += 1

    print(
        f"  Questions with all quotes verified: {questions_with_all_quotes_found}/{len(question_results)} ({questions_with_all_quotes_found/len(question_results)*100:.1f}%)"
    )
    ...
    if transcript_not_found > 0:
        ...
        missing_transcripts = [
            r for r in results if r.match_type == "transcript_not_found"
        ]
        for result in missing_transcripts:
            print(f"  • {result.question_id}: {result.transcript_title}")
...
def main():
    ...
    ground_truth_path = Path(__file__).parent / "base_ground_truth.json"
    data_path = base_path / "1_transcripts" / "jake" / "data.json"

    print("🚀 Ground Truth Source Quote Verification Tool")
    ...
    # Run verification with optional debug mode
    import sys

    debug_mode = "--debug" in sys.argv

    results = verify_quotes(ground_truth_path, data_path, debug_mode=debug_mode)

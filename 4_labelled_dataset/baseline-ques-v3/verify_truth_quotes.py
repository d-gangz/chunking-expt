...
Adapted from verify_source_quotes.py to handle the ground truth question structure.
...
def verify_quotes(ground_truth_path: Path, data_path: Path, debug_mode: bool = False) -> List[QuoteMatch]:
...
def generate_report(results: List[QuoteMatch]) -> None:
    ...
    print(f"  Questions with all quotes verified: {questions_with_all_quotes_found}/{len(question_results)} ({questions_with_all_quotes_found/len(question_results)*100:.1f}%)")
